...
export class CustomNgxDatetimeAdapter extends NgxMatDateAdapter<Moment> {
  ...
  setLocale(locale: string): void {
    ...
    const momentLocaleData = _moment.localeData(locale);
    this._localeData = {
  ...
  today(): Moment {
    return this._createMoment(this.locale as string);
  }

  parse(value: Moment, parseFormat: string | string[]): Moment | null {
  ...
  deserialize(value: Moment): Moment | null {
    let date;
    ...
    if (typeof value === 'string') {
      ...
      date = this._createMoment(value, _moment.ISO_8601).locale(this.locale);
    }
  ...
  isDateInstance(obj: Moment): boolean {
  ...
  private _createMoment(
    ...
    return useUtc
      ? _moment.utc(date, format, locale, strict)
      : _moment(date, format, locale, strict);
  }
}
